...
<body>
    ...
    <div class="w-100 d-flex justify-content-center align-items-center flex-column">
        <form method="POST" action="{{ route('SignupFromLink') }}" enctype="multipart/form-data"
            class="py-5 my-4 text-center  rounded shadow form w-50 ">
            @csrf
            <h1 class="h3 mb-4 "> التسجيل من خلال رابط الدعوة</h1>
            ...
            @error('password')
                <div class="alert alert-danger  w-75 mx-auto">{{ $message }}</div>
            @enderror
            <input id="invited_id" name="invited_id" value="{{$invited_id}}" type="hidden" placeholder=""
                class="form-control my-2 w-75 mx-auto">
